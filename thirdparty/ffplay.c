...
static int opt_add_vfilter(void* optctx, const char* opt, const char* arg) {
    ...
    if (!vfilters_list[nb_vfilters - 1]) return AVERROR(ENOMEM);
...
static inline int cmp_audio_fmts(enum AVSampleFormat fmt1, int64_t channel_count1,
    ...
    else return channel_count1 != channel_count2 || fmt1 != fmt2;
}
...
static int decoder_init(
    Decoder* d, AVCodecContext* avctx, PacketQueue* queue, SDL_cond* empty_queue_cond) {
...
static Frame* frame_queue_peek_last(FrameQueue* f) { return &f->queue[f->rindex]; }
...
static void get_sdl_pix_fmt_and_blendmode(
...
static int upload_texture(SDL_Texture** tex, AVFrame* frame) {
    ...
    case SDL_PIXELFORMAT_IYUV:
        if (frame->linesize[0] > 0 && frame->linesize[1] > 0 && frame->linesize[2] > 0) {
            ...
            av_log(
                NULL, AV_LOG_ERROR, "Mixed negative and positive linesizes are not supported.\n");
            return -1;
        }
        break;
    default:
...
static void set_sdl_yuv_conversion_mode(AVFrame* frame) {
    ...
    if (frame
        ...
        if (frame->color_range == AVCOL_RANGE_JPEG) mode = SDL_YUV_CONVERSION_JPEG;
        else if (frame->colorspace == AVCOL_SPC_BT709) mode = SDL_YUV_CONVERSION_BT709;
...
static void video_image_display(VideoState* is) {
    ...
    calculate_display_rect(
        rect, is->xleft, is->ytop, is->width, is->height, vp->width, vp->height, vp->sar);
    if (vk_renderer) {
            ...
            if (vp->pts >= sp->pts + ((float)sp->sub.start_display_time / 1000)) {
                if (!sp->uploaded) {
                    ...
                    for (i = 0; i < sp->sub.num_rects; i++) {
                        ...
                        is->sub_convert_ctx = sws_getCachedContext(is->sub_convert_ctx, sub_rect->w,
                            sub_rect->h, AV_PIX_FMT_PAL8, sub_rect->w, sub_rect->h, AV_PIX_FMT_BGRA,
                            0, NULL, NULL, NULL);
                        if (!is->sub_convert_ctx) {
                            av_log(
                                NULL, AV_LOG_FATAL, "Cannot initialize the conversion context\n");
                            return;
                        }
                        if (!SDL_LockTexture(
    ...
    SDL_RenderCopyEx(
        renderer, is->vid_texture, NULL, rect, 0, NULL, vp->flip_v ? SDL_FLIP_VERTICAL : 0);
    set_sdl_yuv_conversion_mode(NULL);
    if (sp) {
...
static void video_audio_display(VideoState* s) {
    ...
    if (s->show_mode == SHOW_MODE_WAVES) {
        ...
        if (rdft_bits != s->rdft_bits) {
            ...
            err          = av_tx_init(
                &s->rdft, &s->rdft_fn, AV_TX_FLOAT_RDFT, 0, 1 << rdft_bits, &rdft_scale, 0);
        }
        if (err < 0 || !s->rdft_data) {
            ...
            if (!SDL_LockTexture(s->vis_texture, &rect, (void**)&pixels, &pitch)) {
                ...
                for (y = 0; y < s->height; y++) {
                    ...
                    int a    = sqrt(
                        w * sqrt(data[0][y].re * data[0][y].re + data[0][y].im * data[0][y].im));
...
static void do_exit(VideoState* is) {
    ...
    if (vk_renderer) vk_renderer_destroy(vk_renderer);
...
static void video_display(VideoState* is) {
    ...
    if (is->audio_st && is->show_mode != SHOW_MODE_VIDEO) video_audio_display(is);
    else if (is->video_st) video_image_display(is);
...
static int get_master_sync_type(VideoState* is) {
    if (is->av_sync_type == AV_SYNC_VIDEO_MASTER) {
        if (is->video_st) return AV_SYNC_VIDEO_MASTER;
        else return AV_SYNC_AUDIO_MASTER;
    } else if (is->av_sync_type == AV_SYNC_AUDIO_MASTER) {
        if (is->audio_st) return AV_SYNC_AUDIO_MASTER;
...
static void check_external_clock_speed(VideoState* is) {
    if (is->video_stream >= 0 && is->videoq.nb_packets <= EXTERNAL_CLOCK_MIN_FRAMES
        ...
        if (speed != 1.0)
            set_clock_speed(
                &is->extclk, speed + EXTERNAL_CLOCK_SPEED_STEP * (1.0 - speed) / fabs(1.0 - speed));
    }
}
...
static void video_refresh(void* opaque, double* remaining_time) {
    ...
    if (show_status) {
        ...
        if (!last_time || (cur_time - last_time) >= 30000) {
            ...
            else if (is->video_st) av_diff = get_master_clock(is) - get_clock(&is->vidclk);
            else if (is->audio_st) av_diff = get_master_clock(is) - get_clock(&is->audclk);

            av_bprint_init(&buf, 0, AV_BPRINT_SIZE_AUTOMATIC);
            av_bprintf(&buf, "%7.2f %s:%7.3f fd=%4d aq=%5dKB vq=%5dKB sq=%5dB \r",
                get_master_clock(is),
            ...
            else av_log(NULL, AV_LOG_INFO, "%s", buf.str);
...
static int configure_video_filters(
    AVFilterGraph* graph, VideoState* is, const char* vfilters, AVFrame* frame) {
    ...
    while ((e = av_dict_iterate(sws_dict, e))) {
        ...
        } else av_strlcatf(sws_flags_str, sizeof(sws_flags_str), "%s=%s:", e->key, e->value);
    }
    ...
    filt_out =
        avfilter_graph_alloc_filter(graph, avfilter_get_by_name("buffersink"), "ffplay_buffersink");
    if (!filt_out) {
        ret = AVERROR(ENOMEM);
        goto fail;
    }

    if ((ret = av_opt_set_array(filt_out, "pixel_formats", AV_OPT_SEARCH_CHILDREN, 0, nb_pix_fmts,
             AV_OPT_TYPE_PIXEL_FMT, pix_fmts))
        < 0)
        goto fail;
    if (!vk_renderer
        && (ret = av_opt_set_array(filt_out, "colorspaces", AV_OPT_SEARCH_CHILDREN, 0,
                FF_ARRAY_ELEMS(sdl_supported_color_spaces), AV_OPT_TYPE_INT,
                sdl_supported_color_spaces))
            < 0)
        goto fail;

    if ((ret = av_opt_set_array(filt_out, "alphamodes", AV_OPT_SEARCH_CHILDREN, 0,
             FF_ARRAY_ELEMS(sdl_supported_alpha_modes), AV_OPT_TYPE_INT, sdl_supported_alpha_modes))
        < 0)
        goto fail;
...
static int configure_audio_filters(VideoState* is, const char* afilters, int force_output_format) {
    ...
    ret = snprintf(asrc_args, sizeof(asrc_args),
        "sample_rate=%d:sample_fmt=%s:time_base=%d/%d:channel_layout=%s", is->audio_filter_src.freq,
        av_get_sample_fmt_name(is->audio_filter_src.fmt), 1, is->audio_filter_src.freq, bp.str);

    ret = avfilter_graph_create_filter(
        &filt_asrc, avfilter_get_by_name("abuffer"), "ffplay_abuffer", asrc_args, NULL, is->agraph);
    if (ret < 0) goto end;

    filt_asink = avfilter_graph_alloc_filter(
        is->agraph, avfilter_get_by_name("abuffersink"), "ffplay_abuffersink");
    if (!filt_asink) {
        ret = AVERROR(ENOMEM);
        goto end;
    }
    ...
    if (force_output_format) {
        if ((ret = av_opt_set_array(filt_asink, "channel_layouts", AV_OPT_SEARCH_CHILDREN, 0, 1,
                 AV_OPT_TYPE_CHLAYOUT, &is->audio_tgt.ch_layout))
            < 0)
            goto end;
        if ((ret = av_opt_set_array(filt_asink, "samplerates", AV_OPT_SEARCH_CHILDREN, 0, 1,
                 AV_OPT_TYPE_INT, &is->audio_tgt.freq))
            < 0)
            goto end;
    }
        ...
        if (got_frame) {
            ...
            if (reconfigure) {
                ...
                av_log(NULL, AV_LOG_DEBUG,
                    "Audio frame changed from rate:%d ch:%d fmt:%s layout:%s serial:%d to rate:%d "
                    "ch:%d fmt:%s layout:%s serial:%d\n",
...
static int video_thread(void* arg) {
    ...
    for (;;) {
        ...
        while (ret >= 0) {
            ...
            ret      = queue_picture(
                is, frame, pts, duration, fd ? fd->pkt_pos : -1, is->viddec.pkt_serial);
            av_frame_unref(frame);
...
static int audio_decode_frame(VideoState* is) {
    ...
    data_size = av_samples_get_buffer_size(
        NULL, af->frame->ch_layout.nb_channels, af->frame->nb_samples, af->frame->format, 1);
    ...
    if (af->frame->format != is->audio_src.fmt
        ...
        int ret;
        swr_free(&is->swr_ctx);
        ret = swr_alloc_set_opts2(&is->swr_ctx, &is->audio_tgt.ch_layout, is->audio_tgt.fmt,
            is->audio_tgt.freq, &af->frame->ch_layout, af->frame->format, af->frame->sample_rate, 0,
            NULL);
        if (ret < 0 || swr_init(is->swr_ctx) < 0) {
            av_log(NULL, AV_LOG_ERROR,
                "Cannot create sample rate converter for conversion of %d Hz %s %d channels to %d "
                "Hz %s %d channels!\n",
    ...
    if (is->swr_ctx) {
        ...
        int out_size = av_samples_get_buffer_size(
            NULL, is->audio_tgt.ch_layout.nb_channels, out_count, is->audio_tgt.fmt, 0);
        int len2;
        if (out_size < 0) {
        ...
        resampled_data_size =
            len2 * is->audio_tgt.ch_layout.nb_channels * av_get_bytes_per_sample(is->audio_tgt.fmt);
    } else {
...
static int audio_open(void* opaque, AVChannelLayout* wanted_channel_layout, int wanted_sample_rate,
    ...
    audio_hw_params->bytes_per_sec = av_samples_get_buffer_size(NULL,
        audio_hw_params->ch_layout.nb_channels, audio_hw_params->freq, audio_hw_params->fmt, 1);
...
static int create_hwaccel(AVBufferRef** device_ctx) {
    ...
    *device_ctx = NULL;
    ...
    if (type == AV_HWDEVICE_TYPE_NONE) return AVERROR(ENOTSUP);
...
static int stream_component_open(VideoState* is, int stream_index) {
    ...
    ret = filter_codec_opts(
        codec_opts, avctx->codec_id, ic, ic->streams[stream_index], codec, &opts, NULL);
    if (ret < 0) goto fail;
...
static int read_thread(void* arg) {
    ...
    for (;;) {
        ...
        if (show_status
            ...
            fprintf(stderr, "\x1b[2K\r");
            snprintf(metadata_description, sizeof(metadata_description),
                "\r  New metadata for stream %d", pkt->stream_index);
            dump_dictionary(NULL, ic->streams[pkt->stream_index]->metadata, metadata_description,
                "    ", AV_LOG_INFO);
        }
        ic->streams[pkt->stream_index]->event_flags &= ~AVSTREAM_EVENT_FLAG_METADATA_UPDATED;
...
static VideoState* stream_open(const char* filename, const AVInputFormat* iformat) {
    ...
    if (video_background) {
        if (!strcmp(video_background, "none")) {
            ...
            if (av_parse_color(is->render_params.video_background_color, video_background, -1, NULL)
                >= 0)
...
static void event_loop(VideoState* cur_stream) {
    ...
    for (;;) {
        ...
        case SDL_KEYDOWN:
            ...
            // If we don't yet have a window, skip all key events, because read_thread might still
            // be initializing...
        ...
        case SDL_WINDOWEVENT:
            ...
            case SDL_WINDOWEVENT_SIZE_CHANGED:
                ...
                if (vk_renderer) vk_renderer_resize(vk_renderer, screen_width, screen_height);
...
static int opt_sync(void* optctx, const char* opt, const char* arg) {
    if (!strcmp(arg, "audio")) av_sync_type = AV_SYNC_AUDIO_MASTER;
    else if (!strcmp(arg, "video")) av_sync_type = AV_SYNC_VIDEO_MASTER;
    else if (!strcmp(arg, "ext")) av_sync_type = AV_SYNC_EXTERNAL_CLOCK;
    else {
...
static int opt_input_file(void* optctx, const char* filename) {
    ...
    if (!input_filename) return AVERROR(ENOMEM);
...
static const OptionDef options[] = {
    CMDUTILS_COMMON_OPTIONS { "x", OPT_TYPE_FUNC, OPT_FUNC_ARG, { .func_arg = opt_width },
        "force displayed width", "width" },
    { "y", OPT_TYPE_FUNC, OPT_FUNC_ARG, { .func_arg = opt_height }, "force displayed height",
        "height" },
    ...
    { "ast", OPT_TYPE_STRING, OPT_EXPERT, { &wanted_stream_spec[AVMEDIA_TYPE_AUDIO] },
        "select desired audio stream", "stream_specifier" },
    { "vst", OPT_TYPE_STRING, OPT_EXPERT, { &wanted_stream_spec[AVMEDIA_TYPE_VIDEO] },
        "select desired video stream", "stream_specifier" },
    { "sst", OPT_TYPE_STRING, OPT_EXPERT, { &wanted_stream_spec[AVMEDIA_TYPE_SUBTITLE] },
        "select desired subtitle stream", "stream_specifier" },
    { "ss", OPT_TYPE_TIME, 0, { &start_time }, "seek to a given position in seconds", "pos" },
    { "t", OPT_TYPE_TIME, 0, { &duration }, "play  \"duration\" seconds of audio/video",
        "duration" },
    { "bytes", OPT_TYPE_INT, 0, { &seek_by_bytes }, "seek by bytes 0=off 1=on -1=auto", "val" },
    { "seek_interval", OPT_TYPE_FLOAT, 0, { &seek_interval },
        "set seek interval for left/right keys, in seconds", "seconds" },
    ...
    { "volume", OPT_TYPE_INT, 0, { &startup_volume }, "set startup volume 0=min 100=max",
        "volume" },
    ...
    { "drp", OPT_TYPE_INT, OPT_EXPERT, { &decoder_reorder_pts },
        "let decoder reorder pts 0=off 1=on -1=auto", "" },
    { "lowres", OPT_TYPE_INT, OPT_EXPERT, { &lowres }, "", "" },
    { "sync", OPT_TYPE_FUNC, OPT_FUNC_ARG | OPT_EXPERT, { .func_arg = opt_sync },
    ...
    { "exitonmousedown", OPT_TYPE_BOOL, OPT_EXPERT, { &exit_on_mousedown }, "exit on mouse down",
        "" },
    { "loop", OPT_TYPE_INT, OPT_EXPERT, { &loop },
    ...
    { "vf", OPT_TYPE_FUNC, OPT_FUNC_ARG | OPT_EXPERT, { .func_arg = opt_add_vfilter },
    ...
    { "showmode", OPT_TYPE_FUNC, OPT_FUNC_ARG, { .func_arg = opt_show_mode },
        "select show mode (0 = video, 1 = waves, 2 = RDFT)", "mode" },
    { "i", OPT_TYPE_BOOL, 0, { &dummy }, "read specified file", "input_file" },
    { "codec", OPT_TYPE_FUNC, OPT_FUNC_ARG, { .func_arg = opt_codec }, "force decoder",
        "decoder_name" },
    { "acodec", OPT_TYPE_STRING, OPT_EXPERT, { &audio_codec_name }, "force audio decoder",
        "decoder_name" },
    { "scodec", OPT_TYPE_STRING, OPT_EXPERT, { &subtitle_codec_name }, "force subtitle decoder",
        "decoder_name" },
    { "vcodec", OPT_TYPE_STRING, OPT_EXPERT, { &video_codec_name }, "force video decoder",
        "decoder_name" },
...
void show_help_default(const char* opt, const char* arg) {
    ...
    printf("\nWhile playing:\n"
           ...
           "left/right          seek backward/forward by 10 seconds or a custom interval if "
           "-seek_interval is set\n"
           "down/up             seek backward/forward 1 minute\n"
           "page down/page up   seek to previous/next chapter or backward/forward 10 minutes if no "
           "chapters\n"
...
int main(int argc, char** argv) {
    ...
    if (!display_disable) {
        ...
        if (vk_renderer) {
            ...
            if (ret < 0) {
                av_log(
                    NULL, AV_LOG_FATAL, "Failed to create vulkan renderer, %s\n", av_err2str(ret));
                do_exit(NULL);
            }
        } else {
            ...
            if (!renderer || !renderer_info.num_texture_formats) {
                av_log(
                    NULL, AV_LOG_FATAL, "Failed to create window or renderer: %s", SDL_GetError());
                do_exit(NULL);
            }
        }
    }
